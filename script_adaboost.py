...
def append_record(record):
    with open('./results/individual_results_adaboost.txt', 'w') as f:
        f.write(str(record))
        f.write(os.linesep)
...
for name in os.listdir(path) :
  if name.endswith(".csv") :
    for i in range(10) :
      ...
      clf2 = AdaBoostClassifier(n_estimators=50, base_estimator=clf)
      clf2.fit(train_x, train_y)

      pred_y2 = clf2.predict(test_x)
      report2 = classification_report(test_y, pred_y2, output_dict=True)
      disp = plot_confusion_matrix(clf2, test_x, test_y,
                                    display_labels=[0, 1],
                                    cmap=plt.cm.Blues)
      precision = report2['macro avg']['precision']
      recall = report2['macro avg']['recall']
      score = report2['accuracy']
      f1 = report2['macro avg']['f1-score']
      auc = roc_auc_score(test_y, clf2.predict_proba(test_x)[:, 1])

      tp = disp.confusion_matrix[1][1]
      fp = disp.confusion_matrix[0][1]
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adaboost"]['rf'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adaboost"]['dt'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adaboost"]['nb'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adaboost"]['lr'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
...
with open('./results/averaged_results_adaboost.txt', 'w') as f:
