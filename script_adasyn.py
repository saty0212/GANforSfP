...
def append_record(record):
    with open('./results/individual_results_adasyn.txt', 'w') as f:
        f.write(str(record))
        f.write(os.linesep)
...
for name in os.listdir(path) :
  if name.endswith(".csv") :
    for i in range(10) :
      ...
      adasyn = pd.read_csv(train_path, header=None)
      test = pd.read_csv(test_path, header=None)
      
      test_y = test[test.columns[-1]]
      test_x = test.drop([test.columns[-1]], axis=1)

      adasyn_y = adasyn[adasyn.columns[-1]]
      adasyn_x = adasyn.drop([adasyn.columns[-1]], axis=1)

      ada = ADASYN()
      adasyn_x, adasyn_y = ada.fit_sample(adasyn_x, adasyn_y)

      clf = KNeighborsClassifier(5)
      clf.fit(adasyn_x, adasyn_y)
      ...
      results[name][i+1]["adasyn"]['knn'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adasyn"]['rf'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adasyn"]['dt'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adasyn"]['nb'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
      ...
      fn = disp.confusion_matrix[1][0]
      tn = disp.confusion_matrix[0][0]
      results[name][i+1]["adasyn"]['lr'] = [score, precision, recall, f1, auc, tp, fp, tn, fn]
...
with open('./results/averaged_results_adasyn.txt', 'w') as f:
